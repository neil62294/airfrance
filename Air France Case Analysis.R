...
AirFdf <- within(AirFdf, ROA <- Amount/Total_Cost)
...
AFdf$Book_Prob <- ( AFdf$`Trans. Conv. %` / AFdf$`Engine Click Thru %`) / 10000
...
x <- c('Kayak', 'MSN-Global', 'Yahoo-US', 'Google-Global', 'Overture-Global', 'Google-US', 'Overture-US')
y <- c(64.5, 10.65, 10.35, 4.69, 4.48, 1.22, 1.22)
data <- data.frame(x, y)
...
p <- plot_ly(AFdf_pivot2, x = ~avg_prob, y = ~avg_ROA,
  ...
  layout(title = 'Publisher Strategy',
         xaxis = list(title = "Probability of Booking", showgrid = TRUE, range = c(2.913170e-11, 3.5e-05)),
